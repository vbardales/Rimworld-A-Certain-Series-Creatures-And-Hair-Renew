...
remaining:
  ...
  - first game run, 2026-09-24 (pass 1 English, requested 20260924-164530-619-a69c, on 56ef72d): the run was cut short when the dispatcher's worker died (its game kept running, report not copied). At 11 of 20 scenarios, 6 passed and 5 failed, all five with the same log error, `Tried 300 times to generate age`, which comes from both creature kinds setting `maxGenerationAge` to 0, a defect inherited from 1.0 (the final report, read when the game ends, may add to this). Fixed in the working tree (kinds no longer set either field, `Test-Mod.ps1` group added and mutation-tested); not replayed, so nothing here is confirmed until a pass runs on the fixed revision. The English pass must be redone whole
