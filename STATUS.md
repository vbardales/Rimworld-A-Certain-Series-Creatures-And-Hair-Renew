...
remaining:
  ...
  - protocols read 2026-09-25: `docs/PROTOCOLS-READ.md` says which documents were read, in which version, which were of no use now, and `docs/Check-ProtocolsRead.ps1` lists the ones that moved. Found by that reading, **none done** (the tree is frozen while a request waits): `ATTRIBUTION.md` and its copy in `Mod/` are behind the code (two more corrected defects, the optional Animal Prosthetics 2 patch); the description needs `THANKS` naming that mod's author, Pickle, PickleTools and the AI tool by name; `PUBLICATION.md` does not exist yet; all three belong to `prepublished`
  ...
  - **2026-09-25 evening, in the queue, tree of `83232d1` (Mod/ unchanged since `0aa961c`):** ticket `421e` (features 05 and 06, English) shows the two captures that did not show their subject (a new local step, `A Certain Series: I scroll the research window to the project`, and the hairstyle on the character tab); ticket `3af4` is the fourth pass, feature 15 only, with Animal Prosthetics 2 mounted (the item was already in the Windows Workshop folder and in the WSL cache `~/workshop-cache`, no download was needed: "A Dog Said... Animal Prosthetics 2" is that one mod, ADS being its short name). The Chinese complete pass `d6ae` was ABANDONED by the dispatcher (the Claude application restarted at 19:58, the worker died, nothing was played, no verdict); to be filed again, with `-RunTimeoutMinutes 120`, once `421e` is read, because feature 06 now carries the new step and a red there would waste a complete pass. The tree is frozen until both `RUN_DONE`
  ...
  - unverified: the optional compatibility with A Dog Said... Animal Prosthetics 2 (2026-09-24) is proved offline only (`Test-Mod.ps1`, mutation-tested). Feature 15 and the fourth pass (`wsl-deps.avec-ads2.map`) are written and have not run; the other mod is in the WSL install's Workshop cache (found 2026-09-25); ticket `3af4` plays feature 15. Whether the beetle's vanilla parts (eyes, antennae) have a surgery in that mod is not verified
